...
void ስቅለት(int y), ትንሣኤ(int y), ደብረ_ዘይት(int y), ሆሳዕና(int y), ጰራቅሊጦስ(int y), ዕርገት(int y); // Holidays
...
void Date_Conversion()
{
    ...
    int arr[14][4] = {{}, {20 - leap_c, 10 + leap_c, 9, 7}, {21 - leap_c, 10 + leap_c, 10, 7}, 
        ...
        if (input.day >= 1 && input.day <= arr[input.month][0])
            output = Date{input.day + arr[input.month][1], arr[input.month][2], input.year + arr[input.month][3]};
        else if (input.day >= arr[input.month][0]+1 && input.day <= 30)
            output = Date{input.day - arr[input.month][0], arr[input.month][2]+1, input.year + arr[input.month][3]};
    system("cls");
    wcout << line << endl
          << endl;
...
void ትንሣኤ(int y)
{
    ...
    if (d > 30)
        wcout << L"ትንሣኤ ሚያዝያ " << d % 30 << L" ይውላል።!\n";
    else
        wcout << L"ትንሣኤ መጋቢት " << d << L" ይውላል።!\n";
}
void ዕርገት(int y)
{
    int d;
